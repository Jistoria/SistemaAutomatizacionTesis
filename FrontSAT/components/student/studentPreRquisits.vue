...
const openInfoModal = (index) => {
  const requirement = studentStore.prerequisitos[index];
...
const submitRequirement = async (index) => {
  ...
  if (file) {
    ...
    formData.append("requirementStudentId", studentStore.prerequisitos[index].student_prerequirements_id);
    for (let [key, value] of formData.entries()) {
      console.log(`${key}: ${value.name || value}`);
    }
    openAnimation("spinner");
      await studentStore.sendPreRequeriments(authStore.token, formData);
    closeAnimation();
  ...
  <div v-if="isStudentModalOpen" class="modal modal-open flex items-center justify-center bg-gray-800 bg-opacity-75 fixed inset-0 z-50">
    <div class="modal-box max-w-3xl p-6 bg-white shadow-lg rounded-lg">
      ...
      <table class="table w-full border-collapse">
        ...
        <tbody>
          <tr
            v-for="(requirement, index) in studentStore.prerequisitos"
            :key="index"
            :class="{
